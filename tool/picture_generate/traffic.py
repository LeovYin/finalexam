...
output_path = r"D:\PythonProject\traffic "  # 保存图片的路径
...
try:
    ...
    # 删除水位和漂浮鱼卵数为空的数据
    data = data.dropna(subset=['流量', '漂浮鱼卵数'])
    ...
    for year in data['年份'].unique():
        ...
        plt.title(f"{year} 流量对漂浮鱼卵数量的影响", fontsize=16, color='blue')
        plt.xlabel("流量 ", fontsize=12)
        plt.ylabel("漂浮鱼卵数", fontsize=12)
        plt.ylim(0, 800)  # 限制纵坐标范围
        plt.legend(fontsize=12)
        plt.grid(alpha=0.3)
        plt.savefig(f"{output_path}\\{year}_流量影响图.png", bbox_inches='tight')  # 保存为PNG文件
        plt.close()
